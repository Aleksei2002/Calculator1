...
ALTER TABLE numbers
ADD CONSTRAINT PK_numbers PRIMARY KEY (class);
...
-- 2 Задание --
------- 1 -------
SELECT class, num_id AS "Available Rooms", number_of_seats AS "Number of Seats"
FROM numbers
...
-- 2 Задание --
------- 2 -------
SELECT class, num_id AS "Booked Rooms", number_of_seats AS "Number of Seats"
FROM numbers 
LEFT JOIN guests ON num_id = borrowed_num
...
SELECT *
FROM guests
WHERE '2023-06-30' BETWEEN move_in_date AND move_out_date;
...
-- 3 Задание --
------- 3 -------
CREATE VIEW RoomsWithAvailableSeats AS
SELECT
    gender AS guest_gender,
    num_id AS room_number,
    (number_of_seats - COUNT(passport_num)) AS available_seats
FROM numbers
LEFT JOIN guests ON num_id = borrowed_num
GROUP BY gender, num_id, number_of_seats
